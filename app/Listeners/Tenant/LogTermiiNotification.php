...
class LogTermiiNotification
{
  ...
  public function handle(NotificationSent $event)
  {
    if ($this->notificationIsLoggable($event->notification, $event->channel)) {
      $dbnotification = DatabaseNotification::query()->find($event->notification->id);
      if (blank($dbnotification)) return;

      $response = $this->extractResponse($event->response);
      $this->updateDBNotificationStatus($dbnotification, $response);

      if (is_array($response)) {
        $this->logNotificationToRedis($dbnotification, $response);
      }
    }
  }
   ...
   */
  public function updateDBNotificationStatus(DatabaseNotification $dbnotification, array|string $data)
  {
    return $dbnotification
      ->forceFill([
        'data->status' => $this->hasKey($data, 'messagestate') ? $data['messagestate'] : 'unknown',
        'data->response' => $this->hasKey($data, 'payload') ? $data['payload'] : $data,
      ])->save();
  }
   ...
   */
  public function logNotificationToRedis(DatabaseNotification $dbnotification, array $data)
  {
    ...
    if (filled($messageId) && ($tenant_id = tenant('id'))) {
      Redis::sadd($messageId,  [$tenant_id, $dbnotification->id]);
